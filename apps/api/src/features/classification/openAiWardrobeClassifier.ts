/**
 * 용도:
 * 옷 사진에서 카테고리와 색상, 추천과 유사도 비교에 사용할 관찰 속성을 추출한다.
 *
 * 동작 방식:
 * 이미지에서 직접 확인되는 정보만 구조화하고, 하의는 제품명을 추측하는 대신
 * 바지통·포켓·허리·앞주름 형태를 별도로 기록한다.
 */
import type {
...
const bottomLegShapes = ['skinny', 'straight', 'wide', 'tapered', 'flared', 'unknown'] as const
const pocketStyles = ['none', 'slant', 'welt', 'patch', 'cargo', 'kangaroo', 'zippered', 'mixed', 'unknown'] as const
const necklineStyles = ['crew', 'vNeck', 'mock', 'turtleneck', 'collar', 'hood', 'scoop', 'boat', 'square', 'other', 'unknown'] as const
const frontOpeningStyles = ['none', 'buttons', 'halfButtons', 'zipper', 'halfZip', 'wrap', 'other', 'unknown'] as const
const bottomWaistStyles = ['structured', 'elastic', 'drawstring', 'mixed', 'unknown'] as const
...
const bottomStructureAnalysisGuide = [
  '상의·아우터·중간 레이어·원피스는 necklineStyle, frontOpeningStyle, pocketStyle을 사진에서 관찰해 기록하세요. 하의는 pocketStyle, bottomLegShape, bottomWaistStyle, bottomFrontPleats를 기록하세요. 해당되지 않는 속성은 unknown으로 응답하세요.',
  'necklineStyle은 크루넥 crew, 브이넥 vNeck, 모크넥 mock, 터틀넥 turtleneck, 셔츠나 폴로 형태의 칼라 collar, 후드 hood처럼 실제 목둘레 구조를 기록하세요.',
  'frontOpeningStyle은 앞면 전체 단추 buttons, 목 아래 일부 단추 halfButtons, 전체 지퍼 zipper, 일부 지퍼 halfZip, 랩 여밈 wrap으로 구분하고 앞여밈이 없으면 none을 사용하세요.',
  'pocketStyle은 가장 눈에 띄는 포켓 구조를 기록하세요. 허벅지 옆에 돌출된 덮개형 수납 포켓은 cargo, 겉면에 덧댄 포켓은 patch, 옆선으로 비스듬히 열린 포켓은 slant, 좁은 입술 형태는 welt, 후드 상의 앞면의 연결 포켓은 kangaroo, 지퍼가 핵심이면 zippered입니다. 여러 구조의 비중이 비슷하면 mixed를 사용하세요.',
  'bottomLegShape는 바짓단까지 이어지는 실제 통을 보고 skinny, straight, wide, tapered, flared 중에서 고르세요. 세부 종류 이름을 그대로 복사하지 말고 형태가 불분명하면 unknown을 사용하세요.',
  'bottomWaistStyle은 고정된 허리단과 단추·후크 여밈이면 structured, 밴딩이면 elastic, 끈 조절이 중심이면 drawstring, 두 방식이 함께 뚜렷하면 mixed입니다.',
  'bottomFrontPleats는 허리 아래 앞판에 잡힌 주름이 선명하면 present, 앞판 전체가 충분히 보이고 주름이 없으면 absent, 가림이나 해상도 때문에 판단할 수 없으면 unknown입니다.',
].join(' ')
...
function isFashionItemAttributes(value: unknown): value is FashionItemAttributes {
  ...
  return (
    ...
    typeof candidate.necklineStyle === 'string' &&
    necklineStyles.includes(candidate.necklineStyle) &&
    typeof candidate.frontOpeningStyle === 'string' &&
    frontOpeningStyles.includes(candidate.frontOpeningStyle) &&
    typeof candidate.pocketStyle === 'string' &&
    pocketStyles.includes(candidate.pocketStyle) &&
    typeof candidate.bottomLegShape === 'string' &&
    bottomLegShapes.includes(candidate.bottomLegShape) &&
    typeof candidate.bottomWaistStyle === 'string' &&
    bottomWaistStyles.includes(candidate.bottomWaistStyle) &&
    typeof candidate.bottomFrontPleats === 'string' &&
    trimPresences.includes(candidate.bottomFrontPleats) &&
            ...
            properties: {
              ...
              fashionAttributes: {
                ...
                properties: {
                  ...
                  necklineStyle: { type: 'string', enum: necklineStyles },
                  frontOpeningStyle: { type: 'string', enum: frontOpeningStyles },
                  pocketStyle: { type: 'string', enum: pocketStyles },
                  bottomLegShape: { type: 'string', enum: bottomLegShapes },
                  bottomWaistStyle: { type: 'string', enum: bottomWaistStyles },
                  bottomFrontPleats: { type: 'string', enum: trimPresences },
